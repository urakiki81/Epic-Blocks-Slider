...
var useCompositeGroup = createHook.createHook({
  ...
  propsAreEqual: function propsAreEqual(prev, next) {
    if (!next.id || prev.id !== next.id) {
      return Group.useGroup.unstable_propsAreEqual(prev, next);
    }
    ...
    if (prev.items && next.items) {
      var prevCurrentItem = CompositeState.findEnabledItemById(prev.items, prevCurrentId);
      var nextCurrentItem = CompositeState.findEnabledItemById(next.items, nextCurrentId);
    ...
    return Group.useGroup.unstable_propsAreEqual(prevProps, nextProps);
  },
