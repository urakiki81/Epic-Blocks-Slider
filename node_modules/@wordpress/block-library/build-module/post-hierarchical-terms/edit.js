...
import { AlignmentToolbar, BlockControls, Warning, __experimentalBlock as Block, __experimentalBlockVariationPicker as BlockVariationPicker } from '@wordpress/block-editor';
...
export default function PostHierarchicalTermsEdit(_ref) {
  ...
  if (!hasPost) {
    return createElement(Block.div, null, createElement(Warning, null, __('Post Hierarchical Terms block: post not found.')));
  }

  if (!term) {
    var _blockType$icon;

    return createElement(Block.div, null, createElement(BlockVariationPicker, {
  ...
  }, isLoadingHierarchicalTermLinks && createElement(Spinner, null), hasHierarchicalTermLinks && !isLoadingHierarchicalTermLinks && hierarchicalTermLinks.reduce(function (prev, curr) {
    return [prev, ' | ', curr];
